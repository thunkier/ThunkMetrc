...
use thunkmetrc_wrapper::{MetrcWrapper, MetrcClient, RateLimiterConfig};
use dotenv::dotenv;
use std::env;
...
async fn test_active_inventory_sync() {
    ...
    let license_number = env::var("METRC_FACILITY_LICENSE_NUMBER").expect("METRC_FACILITY_LICENSE_NUMBER not set");
    ...
    let config = RateLimiterConfig {
        enabled: true,
        max_get_per_second_integrator: 20.0,
        max_get_per_second_per_facility: 10.0,
        max_concurrent_get_integrator: 5,
        max_concurrent_get_per_facility: 2,
        max_retries: 3,
    };
    ...
    let result = thunk.active_packages_inventory_sync(
        &license_number,
        None,
        5 // buffer minutes
    ).await;
