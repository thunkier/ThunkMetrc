use thunkmetrc_wrapper::services::patients_service::PatientsService;
...
#[async_trait]
pub trait PatientsServiceExt {

    /// Iterator for get_active_patients.
    fn iterate_get_active_patients<'a>(
        &'a self,
        license_number: Option<&str>, page_size: Option<&str>
    ) -> impl Stream<Item = Result<Patient>> + 'a;
}

#[async_trait]
impl PatientsServiceExt for PatientsService {
    ...
    fn iterate_get_active_patients<'a>(
        ...
        try_stream! {
            ...
            loop {
                ...
                let res = self.get_active_patients(
