  ...
  private async executeRateLimited<T>(facility: string | null, op: () => Promise<T>): Promise<T> {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        // If no Retry-After or 5xx, use Jittered Exponential Backoff
        if (waitMs === 0) {
            const exponential = initialBackoffMs * Math.pow(backoffExponent, attempt);
            const capped = Math.min(exponential, maxBackoffMs);
            // Full Jitter: random between 0 and capped
            // Or Proportional Jitter: capped + random(0-20%)?
            // "Decorrelated Jitter" is often best, but Simple Full Jitter is standard.
            // Let's use Proportional Jitter to match C# logic (base + 0-20% jitter)
            const jitter = Math.random() * 0.2 * capped;
            waitMs = capped + jitter;
        }
