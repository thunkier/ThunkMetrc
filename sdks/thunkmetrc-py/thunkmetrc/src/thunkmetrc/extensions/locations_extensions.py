...
class LocationsExtensions:
    @staticmethod
    async def iterate_get_active_locations(
        service,
        ...
        """
        Iterates through all pages of GetActiveLocations.
        """
        all_results = []
        page = 1
        while True:
            response = await service.get_active_locations(
        ...
        return all_results
    @staticmethod
    async def iterate_get_inactive_locations(
        service,
        license_number: str = None,
        page_size: str = None,
    ) -> List['LocationsLocation']:
        """
        Iterates through all pages of GetInactiveLocations.
        """
        all_results = []
        page = 1
        while True:
            response = await service.get_inactive_locations(
                license_number=license_number,
                page_size=page_size,
                page_number=page
            )
        ...
        return all_results
    @staticmethod
    async def iterate_get_locations_types(
        service,
        license_number: str = None,
        page_size: str = None,
    ) -> List['LocationsType']:
        """
        Iterates through all pages of GetLocationsTypes.
        """
        all_results = []
        page = 1
        while True:
            response = await service.get_locations_types(
                license_number=license_number,
                page_size=page_size,
                page_number=page
            )
    ...
    @staticmethod
    async def sync_get_active_locations(
        service,
        ...
        return await LocationsExtensions.iterate_get_active_locations(
            service,
    ...
    @staticmethod
    async def sync_get_active_locations_parallel(
        targets: List[Tuple['MetrcWrapper', str]],
        last_known_sync: datetime.datetime,
        buffer_minutes: int,
        concurrency_limit: int = 20
    ) -> Dict[str, List['LocationsLocation']]:
        """
        ...
        async def op(wrapper: 'MetrcWrapper', limiter: MetrcRateLimiter, license_number: str):
             ...
             # wrapper.locations
             return await LocationsExtensions.sync_get_active_locations(
                 wrapper.locations,
                 last_known_sync,
                 buffer_minutes,
                 license_number
