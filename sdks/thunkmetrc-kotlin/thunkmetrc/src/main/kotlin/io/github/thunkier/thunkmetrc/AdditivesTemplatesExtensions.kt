...
import io.github.thunkier.thunkmetrc.wrapper.services.AdditivesTemplatesService
...
/**
 * Lazy pagination iterator for getActiveAdditivesTemplates.
 * @return A Flow that yields items lazily across all pages
 */
@Suppress("UNCHECKED_CAST")
fun AdditivesTemplatesService.iterateGetActiveAdditivesTemplates(lastModifiedEnd: String? = null, lastModifiedStart: String? = null, licenseNumber: String? = null, pageSize: String? = null): Flow<AdditivesTemplate> {
    return MetrcExtensions.iteratePages { page ->
        this.getActiveAdditivesTemplates(
                    lastModifiedEnd,
                    lastModifiedStart,
                    licenseNumber,
                    page.toString(),
                    pageSize,) as? PaginatedResponse<AdditivesTemplate>
    }
}
/**
 * Sync items for getActiveAdditivesTemplates.
 ...
 */
suspend fun AdditivesTemplatesService.syncGetActiveAdditivesTemplates(
    ...
    return this.iterateGetActiveAdditivesTemplates(endStr, startStr, licenseNumber, pageSize).toList()
}

/**
 * Sync items in parallel across multiple facilities for getActiveAdditivesTemplates.
 ...
 */
suspend fun syncGetActiveAdditivesTemplatesParallel(
    targets: List<Pair<MetrcClient, String>>,
    lastKnownSync: OffsetDateTime? = null,
    bufferMinutes: Int = 15
    , pageSize: String? = null
): Map<String, List<AdditivesTemplate>> = MetrcExtensions.syncParallel(targets) { client, limiter, license ->
    val svc = AdditivesTemplatesService(client, limiter)
    
    svc.syncGetActiveAdditivesTemplates(
...
/**
 * Lazy pagination iterator for getInactiveAdditivesTemplates.
 * @return A Flow that yields items lazily across all pages
 */
@Suppress("UNCHECKED_CAST")
fun AdditivesTemplatesService.iterateGetInactiveAdditivesTemplates(licenseNumber: String? = null, pageSize: String? = null): Flow<AdditivesTemplate> {
    return MetrcExtensions.iteratePages { page ->
        this.getInactiveAdditivesTemplates(
                    licenseNumber,
                    page.toString(),
                    pageSize,) as? PaginatedResponse<AdditivesTemplate>
    }
