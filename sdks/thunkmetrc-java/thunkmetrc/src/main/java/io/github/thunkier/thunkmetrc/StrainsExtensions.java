...
public class StrainsExtensions {

    /**
     * Iterator for getActiveStrains.
     * Returns an Iterable that fetches pages lazily and yields items.
     */
    @SuppressWarnings("unchecked")
    public static Iterable<Strain> iterateGetActiveStrains(
        StrainsService service,
        String lastModifiedEnd, String lastModifiedStart, String licenseNumber, String pageSize
            ...
            try {
                Object res = service.getActiveStrains(
     ...
     */
    public static List<Strain> syncGetActiveStrains(
        StrainsService service,
        ...
        List<Strain> results = new ArrayList<>();

        for (Strain item : iterateGetActiveStrains(service, endStr, startStr, licenseNumber, pageSize)) {
            results.add(item);
        }
        return results;
    }
     ...
     */
    public static Map<String, List<Strain>> syncGetActiveStrainsParallel(
        ...
        return MetrcExtensions.syncParallel(targets, concurrencyLimit, (client, limiter, license) -> {
            StrainsService svc = new StrainsService(client, limiter);
            return syncGetActiveStrains(
                svc,
    ...
    /**
     * Iterator for getInactiveStrains.
     * Returns an Iterable that fetches pages lazily and yields items.
     */
    @SuppressWarnings("unchecked")
    public static Iterable<Strain> iterateGetInactiveStrains(
        StrainsService service,
        String licenseNumber, String pageSize
    ) {return MetrcExtensions.iteratePages(page -> {
            try {
                Object res = service.getInactiveStrains(
